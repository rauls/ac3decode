...
int main(int argc,char *argv[])
{
	...
	double	time_start, time_end, time_other=0, timesofar=0, x;
	...
	fprintf( stderr, "A(3 V0.8.19 (c) 2000 (www.ac3dec.com, built %s %s)\n", __DATE__, __TIME__  );
	...
	if(argc > 1){
		...
		while( argc > i ){
			...
			if ( !strcmp( argv[i], "-wma" ) ){
				m_waveoutname = argv[i+1];
				//m_fDecodeTarget = _MP3FILE; /* output mode to FILE */
				printf( "WMA(asf) output format has not yet been inplemented\n" );
				i++;
			}
			else
	...
	while( !fUserQuit )
	{
		...
		if ( cbIteration == 0 ) /* first time this loop is run? */
		{
			if ( m_ac3seek && !cbIteration ){
				...
				printf( "Seeking %d Kbytes...\n", seekpos/1024 );
				fseek( g_pInfile, seekpos, SEEK_SET );
				SeekToAC3Header( g_pInfile );
			}
		...
		for(i=0; (i < 6) && (cbErrors < ERR_RESYNC_THRESHOLD ); i++)
		{
			...
			if ( n ) {	/* ########### SHOW TIMECODE AS IT OUTPUTS ########### */
				...
				printf( "\rDUR=%s.%03d (%0.3fsecs)", timeStr, seclength*1000/SampleRate, timelength );
				ct = getms();
				timesofar = ct - time_start;
				if ( 1 ){
					//printf( "(t=%f,%f)", time_start, ct );
					printf( " (%d/sec, 1:%0.1f)    ",
