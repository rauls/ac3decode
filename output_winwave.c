...
void parse_error( char *errtxt, MMRESULT mmresult )
{
...
/*	tmp = SetEvent( EventBox.status.EventPBReady.hE ); */
  ...
  /* open waveOut device, supply a user-defined CALLBACK function, 
	...
	if ( mmresult != MMSYSERR_NOERROR )
	{
		parse_error( "noutput_open() : waveOutOpen()", mmresult );
	...
	mmresult = waveOutGetDevCaps( m_hwo, &waveoutcaps, sizeof( waveoutcaps ) );
...
long output_play(stream_samples_t *samples, bsi_t *pbsi )
{
  ...
  /* AC3DEC generates 1024 bytes (256 stereo samples) per call.
	...
	*/ 
	...
	if ( counter == 0 )
		OutPrintf("\rBytes written,           Duration (HH:MM:SS) (seconds)\n");
  ...
  /* Before a WAVEHDR can be prepared, it must be "clear."  I.e. if it has been
	...
	if ( mmresult != MMSYSERR_NOERROR )
		parse_error( "output_play() : waveOutPrepareHeader()", mmresult );
  ...
  /* 3) write the buffer to the waveOut device.  Play the buffer! */
	...
	if ( mmresult != MMSYSERR_NOERROR )
		parse_error( "output_play() : waveOutWrite()", mmresult );
...
output_close(void)
{
	...
	do {
		freed = output_flush();
		freed_count += freed;
		//printf("\routput_close() : waiting for waveOut, buffer #%d", freed_count+1 );
	...
	if ( mmresult != MMSYSERR_NOERROR )
		parse_error( "output_close() : waveOutReset()", mmresult );


	/* Release the hardware-resource back to Windows */
	mmresult=waveOutClose( m_hwo ); 
	if ( mmresult != MMSYSERR_NOERROR )
		parse_error( "output_close() : waveOutClose()", mmresult );
